...
export const projects: Project[] = [
  {
    ...
    description: "Interactive dashboards to visualize spatial and temporal trends of San Francisco crime.",
  ...
  },
  {
    ...
    overview: "The Pokémon Trading Card Game (TCG) is a dynamic and competitive collectible card game featuring thousands of unique cards, frequent expansions, and an evolving set of tournament-legal cards. In such a rapidly shifting environment, being the first to discover card synergies and identify the strongest cards is vital for competitive success. As the final project for the Natural Language Processing course in my master's program, our group applied modern NLP machine learning techniques to historical tournament data and card metadata to identify synergies between cards and predict card strength before the cards even see any tournament play, providing players with a data-driven edge. Our models aimed to quantify relationships between cards and assess their potential utility in evolving metagames. These findings have applications in competitive analytics, deck-building tools, and strategic planning. The main data sources for this project were a list of all Pokémon cards from 1999 to 2023 (over 17,000 cards) and a list of all competitive Pokémon deck lists used in tournaments from 2011 to 2023. We input the raw text from each of these cards (card type, description, abilities, etc.) into our models. We used the tournament data and time series cross validation to evaluate our model results.",
    implementation: [
  ...
  },
  {
    ...
    overview: "Throughout the Fundamentals of Data Engineering course, we developed a delivery service for a hypothetical company called Acme Gourmet Meals (AGM). The product and purchase data for this company were stored in PostgreSQL relational tables. As the final project for this class, our group created a NoSQL Neo4j database to find optimized routes for food delivery services that took advantage of the BART system. This would be an eco-friendly solution for customers that would still like their food delivered straight to their doors.",
    implementation: [
      ...
      "Applied betweenness centrality to identify key transit stations that act as crucial intermediates in the network",
      "Applied harmonic centrality to identify most accessible transit stations for choosing new AGM locations",
      "Different objectives can be optimized easily by changing edge weights between stations and customers"
    ],
    results: [
...
export const repositories: Repository[] = [
  ...
  },
  {
    ...
    description: "NoSQL Neo4j database to compute optimized food delivery routes using public transportation",
